...
#                                              relay
#
# THE WORKLOAD GATEWAY NEEDS NOTHING RENDERED (TOON_Network#114). It used to:
# this script wrote conf/.rendered/workload-gateway-hs.env, the committed dial
# rewrite plus one entry sending <addr>.anyone:80 at provider-hs:8080, because
# the gateway sent `status` as a plain POST that only the provider APP serves.
# The gateway now sends the sealed packet the Profile describes, which the
# hidden CONNECTOR terminates at exactly the address its Profile advertises —
# so it is reached the honest way, through TOON_SOCKS_PROXY, and a per-provider
# line in a config is one fewer thing this sandbox pretends about.
#
# THREE outputs, where scripts/hs-address.sh renders one, because a HIDDEN
